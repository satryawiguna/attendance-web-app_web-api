<!DOCTYPE html>
<html lang="en">

<head>

  <meta charset="utf-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
  <meta name="description" content="">
  <meta name="author" content="">

  <title>Smart Attendance - Dashboard</title>

  <!-- Custom fonts for this template-->
  <link href="{{asset('dasbor/vendor/fontawesome-free/css/all.min.css')}}" rel="stylesheet" type="text/css">
  <link href="https://fonts.googleapis.com/css?family=Nunito:200,200i,300,300i,400,400i,600,600i,700,700i,800,800i,900,900i" rel="stylesheet">

  <!-- Custom styles for this template-->
  <link href="{{asset('dasbor/css/sb-admin-2.min.css')}}" rel="stylesheet">
  <link href="{{asset('dasbor/vendor/datatables/dataTables.bootstrap4.min.css')}}" rel="stylesheet">
  @yield('custom_css')

</head>

<body id="page-top">

  <!-- Page Wrapper -->
  <div id="wrapper">

    @include('dashboard.layouts.master_sidebar')

    <!-- Content Wrapper -->
    <div id="content-wrapper" class="d-flex flex-column">

      <!-- Main Content -->
      <div id="content">

        @include('dashboard.layouts.master_header')
        
        @yield('content')

      </div>
      <!-- End of Main Content -->

      @include('dashboard.layouts.master_footer')

    </div>
    <!-- End of Content Wrapper -->

  </div>
  <!-- End of Page Wrapper -->

  <!-- Scroll to Top Button-->
  <a class="scroll-to-top rounded" href="#page-top">
    <i class="fas fa-angle-up"></i>
  </a>

  <!-- Bootstrap core JavaScript-->
  <script src="{{asset('dasbor/vendor/jquery/jquery.min.js')}}"></script>
  <script src="{{asset('dasbor/vendor/bootstrap/js/bootstrap.bundle.min.js')}}"></script>

  <!-- Core plugin JavaScript-->
  <script src="{{asset('dasbor/vendor/jquery-easing/jquery.easing.min.js')}}"></script>

  <!-- Custom scripts for all pages-->
  <script src="{{asset('dasbor/js/sb-admin-2.min.js')}}"></script>

  <!-- Page level plugins -->
  <script src="{{asset('dasbor/vendor/chart.js/Chart.min.js')}}"></script>
  <script src="{{asset('dasbor/vendor/datatables/jquery.dataTables.min.js')}}"></script>
  <script src="{{asset('dasbor/vendor/datatables/dataTables.bootstrap4.min.js')}}"></script>

  <script>
    $('form').submit(function() {
      $(this).find('button[type=submit]').prop('disabled', true);
    });
  </script>

  @yield('custom_js')
</body>

</html>
